...
def generate_association_rules(dataset, min_support=0.02, confiance=0.04):
  ...
  items_together = apriori(basket, min_support=min_support, use_colnames=True)
  items_together.sort_values(by=['support'], ascending=False)
  rules = association_rules(items_together, metric="confidence", min_threshold=confiance)
